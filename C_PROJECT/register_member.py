...
class RegisterMember(QtWidgets.QWidget):
    member_registered_successfully = pyqtSignal()
    def __init__(self,the_stack):
        super().__init__()
        uic.loadUi("C_PROJECT/UI_FILES/register_member.ui",self)
    ...
    def add_member_to_db(self):
        ...
        if self.fULLNAMELineEdit.text() and self.nATIONALIDLineEdit.text() and self.pHONENUMBER1LineEdit.text():
            ...
            if success:
                ...
                conn.commit()
                conn.close()
                self.member_registered_successfully.emit()
                self.the_stack.setCurrentIndex(2)
                
            # financial_totals
            initial_contrib = self.membership_fee.text()
